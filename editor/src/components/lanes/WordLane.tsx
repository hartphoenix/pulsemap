...
export function WordLane({
	...
	const visibleWords = useMemo(() => {
		...
		return words.slice(start, end).map((word, i) => {
			const idx = start + i;
			const endMs = word.end ?? (idx + 1 < words.length ? words[idx + 1].t : undefined);
	...
	return (
		<Lane label="Words" height={height}>
			{visibleWords.map((word) => {
				...
				const w = word.endMs ? (word.endMs - word.t) * pxPerMs : 40;
				const selected =
					state.selection?.lane === "words" && state.selection?.index === word.globalIdx;
